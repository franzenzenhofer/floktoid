// Quick test for boss spawning logic
const getBossConfig = (wave) => {
...
// Test waves
console.log("Boss spawning test:");
for (let wave = 1; wave <= 20; wave++) {
  const config = getBossConfig(wave);
  if (config.count > 0) {
    console.log(`Wave ${wave}: ${config.count} boss(es), ${config.health} HP, ${config.shootingPercent}% shooting`);
  } else {
    console.log(`Wave ${wave}: Normal wave (no bosses)`);
  }
}